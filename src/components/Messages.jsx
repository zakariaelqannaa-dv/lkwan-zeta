...
const Messages = ({ user }) => {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      // Combine all contact IDs
      const contactIds = [...new Set([
        ...(following || []).map(f => f.following_id),
        ...(followers || []).map(f => f.follower_id)
      ])];

      // Include anyone you've messaged with
  ...
  const markAsRead = useCallback(async (contactId) => {
    ...
    // Only receiver can mark messages as read (frontend enforcement)
    // Ensure we're only updating messages where current user is receiver
    setContacts(prev => prev.map(c => c.id === contactId ? { ...c, unreadCount: 0 } : c));
    try {
      await supabase.from('messages')
        .update({ is_read: true })
        .eq('sender_id', contactId)
        .eq('receiver_id', user.id)  // Ensure current user is receiver
        .eq('is_read', false);
  ...
  useEffect(() => {
    fetchContacts();
    const channel = supabase.channel(`ig-messages-${user.id}`)
      .on('postgres_changes', { 
        ...
        <header className="shrink-0">
          <div className="flex items-center justify-between px-4 h-[53px] border-b border-[#2f3336]">
            <div className="flex items-center gap-1 sm:gap-0">
              <button type="button" onClick={() => navigate(-1)} className="sm:hidden p-1.5 -ml-1.5 rounded-full hover:bg-[#16181c] transition text-[#e7e9ea]">
                <ChevronLeft size={20} />
              </button>
          ...
          <>
            ...
            <header className="sticky top-0 z-10 px-4 h-[53px] border-b border-[#2f3336] flex items-center justify-between bg-black shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-sm text-[#e7e9ea] inline-flex items-center gap-1">{selectedContact.display_name || selectedContact.username}<VerificationBadge show={isVerified(selectedContact)} size="sm" /></h3>
              </div>
            ...
            {/* Input */}
            <footer className="bg-black border-t border-[#2f3336] px-3 py-3 fixed bottom-0 left-0 right-0 z-20 sm:static sm:z-auto" style={{ paddingBottom: 'max(12px, env(safe-area-inset-bottom, 12px))' }}>
